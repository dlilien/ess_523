...
class Model:
    ...
    def add_IC(self, function, eqn_name=None):
        ...
        if not self.time_dep:
            raise ValueError(
                'Initial condition cannot apply to steady state model')
        try:
        ...
        if isinstance(key, int):
            try:
                return self.mainDict[
                    [keyname for keynum, keyname in self.keyPairs if keynum == key][0]]
            except IndexError:
    ...
    def __init__(self, model, *eqn, **kwargs):
        ...
        if eqn:
            ...
            if 'eqn_name' in kwargs and (kwargs['eqn_name'] is not None):
                ...
                self.eqn_name = [
                    name for num,
                    name in self.parent.eqn.keyPairs if num == self.parent.eqn.numbers[0]][0]
        ...
        if self.eqn.dofs == 1:
            ...
            for i, node1 in self.mesh.nodes.items():
                ...
                for j, node2_els in node1.neighbors.items():
                    ...
                    rows[nnz] = i - 1
                    cols[nnz] = j - 1
                    data[nnz] = self.eqn(i, j, [(nei_el, self.mesh.elements[
                                         nei_el]) for nei_el in node2_els if self.mesh.elements[nei_el].eltypes == 2], **kwargs)
                    nnz += 1

            # store what we have done
            self.matrix = csc_matrix(
                (data, (rows, cols)), shape=(
                    self.mesh.numnodes, self.mesh.numnodes))
            self.rhs = rhs
            return None
            ...
            # Empty vectors to accept the sparse info, make them large for
            # cross terms
            ...
            for i, node1 in self.mesh.nodes.items():
                ...
                for j, node2_els in node1.neighbors.items():
                    ...
                    # dumb things are happening with i,j in the supplement, so
                    # these don't match
                    data[nnz], data[nnz + 1], data[nnz + 2], data[nnz + 3] = self.eqn(
                        i, j, [(nei_el, self.mesh.elements[nei_el]) for nei_el in node2_els if self.mesh.elements[nei_el].eltypes == 2], **kwargs)
    ...
    def applyBCs(self, time=None, normal=True):
        ...
        if not all(edges == np.sort(list(b_funcs.keys()))):
            ...
            for edge in list(b_funcs.keys()):
                ...
                print(
                    'Some border not specified in types, taking equal to zero')
                print('Borders are ', edges, ' listed are ', b_funcs.keys())

        # Ok, hopefully we have parse-able input now
        # Figure out which nodes are associated with each boundary
        edge_nodes = {}
        for edge in edges:
            edge_nodes[edge] = np.zeros(
            ...
            try:
                if time is not None:
                    ...
                    self._applyNeumann(
                        ...
                        b_funcs[edge],
                        normal=normal)
            # If we have no condition we are just taking 0 neumann
            except KeyError:
                pass
                                                ...
                                                self.mesh.elements[el].length * gpt[0] * (
                                                    np.dot(
                                                        function(
                                                            self.mesh.elements[el].F(
                                                                gpt[
                                                                    1:-1])), self.elements[el].normal)) * self.mesh.elements[el].bases[
                                                    self.mesh.elements[el].nodes.index(node)](
                                                    self.mesh.elements[el].F(
                                                        gpt[
            ...
            for node in edge_nodes:
                ...
                for j in self.mesh.nodes[node].neighbors.keys():
                    if j not in edge_nodes:
                        ...
                        self.rhs[j - 1] = self.rhs[j - 1] - \
                            self.matrix[j - 1, node - 1] * self.rhs[node - 1]
    ...
    def solveIt(self):
        ...
        if self.eqn.method == 'CG':
            if self.eqn.precond is not None:
                self.sol, info = cg(
                    self.matrix, self.rhs, tol=self.eqn.lin_tolerance, M=M)
            else:
                self.sol, info = cg(
                    self.matrix, self.rhs, tol=self.eqn.lin_tolerance)
            if info > 0:
                warn(
                    'Conjugate gradient did not converge. Attempting BiCGStab')
                if self.eqn.precond is not None:
                    self.sol, info = bicgstab(
                        self.matrix, self.rhs, tol=self.eqn.lin_tolerance, M=M)
                else:
                    self.sol, info = bicgstab(
                        self.matrix, self.rhs, tol=self.eqn.lin_tolerance)
...
class LinearModel(ModelIterate):
    ...
    def iterate(self, time=None, **kwargs):
        ...
        if time is not None:
            if 'BDF1' in kwargs:
                self.matrix = kwargs[
                    'timestep'] * self.matrix + diags(np.ones(self.mesh.numnodes), 0)
                self.rhs = kwargs['timestep'] * self.rhs + \
                    np.array(kwargs['td_sol'][-1][name])
            elif 'BDF2' in kwargs:
        ...
        try:
            for i in range(self.eqn.nl_maxiter):
                ...
                if relchange < self.eqn.nl_tolerance and i != 0:
                    break
                old = self.eqn.relaxation * \
                    new[:] + (1.0 - self.eqn.relaxation) * old[:]
            else:  # Executed if we never break b/c of convergence
                if abort_not_converged:
                    raise ConvergenceError(
                        'Nonlinear solver failed within iteration count')
                else:
                    print(
                        'Nonlinear solver did not converge within desired tolerance, returning')
            self.sol = new
...
class MultiModel:
    ...
    def iterate(
        ...
        # We are going to have slots for every equation, even if some are
        # linear
        self.sol = {
            name: np.zeros(
                self.model.mesh.numnodes *
                eqn.dofs) for name,
            eqn in self.model.eqn.items() if [
                eqnum for eqnum,
                eqname in self.model.eqn.keyPairs if eqname == name][0] in self.model.eqn.de_numbers}
        relchange = {name: 1.0 for name in self.sol}
        ss_relchange = relchange.copy()
        self.models = OrderedDict([([name for eqnnum, name in self.model.eqn.keyPairs if eqnnum == num][0], self.model.makeIterate(
            [name for eqnnum, name in self.model.eqn.keyPairs if eqnnum == num][0])) for num in sorted(self.model.eqn.numbers)])

        for k in range(0, ss_maxiter):
            for name, model in self.models.items():
                if [eqnum for eqnum, eqname in self.model.eqn.keyPairs if eqname == name][
                        0] in self.model.eqn.de_numbers:
                    ...
                    if k != 0:
                        ss_relchange[name] = np.linalg.norm(
                            self.sol[name] - new[name]) / np.sqrt(float(self.model.mesh.numnodes)) / np.linalg.norm(new[name])
            ...
            if np.all(
                [
                    ss_relchange[name] < eqn.ss_tolerance for name,
                    eqn in self.model.eqn.items() if [
                        eqnum for eqnum,
                        eqname in self.model.eqn.keyPairs if eqname == name][0] in self.model.eqn.de_numbers]) and k != 0:
                print(
                    'Steady State convergence reached at iteration', str(
                        k + 1))
                break
            ...
            if abort_not_converged:
                raise ConvergenceError(
                    'Steady state solver failed within iteration count')
            else:
                print(
                    'Steady state solver did not converge within desired tolerance, returning')
        return self.sol
...
class TimeDependentModel:
    ...
    def __init__(self, model, timestep, n_steps, method='BDF1'):
        ...
        if not n_steps > 0:
            raise ValueError(
                'Number of timesteps must be strictly greater than 0')
    ...
    def iterate(self):
        ...
        """
        sol = [{name: np.array([eqn.IC(pt) for pt in self.model.mesh.coords]) for name,
                eqn in self.model.eqn.items() if [eqnum for eqnum,
                                                  eqname in self.model.eqn.keyPairs if eqname == name][0] in self.model.eqn.de_numbers}]
        if self.method == 'BDF2':
            time = self.timestep
            sol.append(
                self.model.makeIterate().iterate(
                    time=time,
                    BDF1=True,
    ...
    def animate(self, show=False, save=None):
        ...
        ax = p3.Axes3D(fig)
        ax.set_xlim3d(
            [min(self.model.mesh.coords[:, 0]), max(self.model.mesh.coords[:, 0])])
        ax.set_ylim3d(
            [min(self.model.mesh.coords[:, 1]), max(self.model.mesh.coords[:, 1])])
        ...
        class nextPlot:
            ...
            def __call__(self, num):
                ...
                ax.clear()
                ax.set_xlim3d(
                    [min(self.outer.model.mesh.coords[:, 0]), max(self.outer.model.mesh.coords[:, 0])])
                ax.set_ylim3d(
...
class ConvergenceError(Exception):
    ...
    def __str__(self):
        return 'Method ' + self.method + \
            ' did not converge at iteration ' + str(self.iters)
