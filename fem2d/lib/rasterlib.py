    ...
    *DEMs : list of strings
    ...
    def __call__(self, pt):
        if self.splines == 1:
            ...
            if self.subtract:
                return self.spline1(pt[1],
                                    pt[0])[0] - self.spline2(pt[1],
                                                             pt[0])[0]
            else:
                return np.array(
                    [self.spline1(pt[1], pt[0])[0], self.spline2(pt[1], pt[0])[0]])
...
def rasterizeMesh(mesh, raster, var_names, elementwise=True):
    ...
    if not len(var_names) == raster.splines:
        raise ValueError(
            'Number of variable names does not match number of splines')

    if elementwise:
        for elm in mesh.elements.values():
            if elm.eltypes == 2:
                if raster.splines == 1:
                    elm.phys_vars[var_names[0]] = elm.area * 2 * \
                        np.sum([gp[0] * raster(gp[1]) for gp in elm.gpts])
                else:
                    elm.phys_vars[var_names[0]], elm.phys_vars[var_names[
                        1]] = elm.area * 2 * np.sum(np.array([gp[0] * raster(gp[1]) for gp in elm.gpts]), 0)
