...
def MakeMatrix(mesh,equation='area',max_nei=8):
    ...
    for i,node1 in mesh.nodes.items():
        ...
        if type(equation)==str:
            if equation=='area':
                data[nnz]=np.sum([mesh.elements[elm[0]].area for elm in node1.ass_elms if mesh.elements[elm[0]].eltypes==2]) #TODO fix indexing
            elif equation=='masscon':
                pass
            else:
                print 'Bad equation name'
                break
        else:
            print 'Equation not understood'
        nnz += 1
        for j,node2_els in node1.neighbors.items():
            ...
            if type(equation)==str:
                if equation=='area':
                    data[nnz]=np.sum([mesh.elements[nei_el].area for nei_el in node2_els if mesh.elements[nei_el].eltypes==2]) #TODO fix indexing
                else:
                    print 'Bad equation name'
                    break
            else:
